...
const BattleshipsCanvas = ({
  ...
  const handleMouseMove = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (!isDragging && !isDrawing && !isGameTime) {
    ...
    if (ctx) {
      ...
      const isHorizontal =
        heightInCells < 2 && widthInCells >= 2 && widthInCells <= 5;
      const isVertical =
      ...
      xt = Math.max(0, Math.min(x, 550));
      yt = Math.max(0, Math.min(y, 550));
    }
  };
  ...
  const handleMouseUp = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (
      ...
      const largestAvailableShip =
        (availableShipSizes.length ? Math.max(...availableShipSizes) : 0) * 50;
      ...
      const generateSections = (size) => {
        const sections = [];
        console.log('size in GenerateSections', size, isHorizontalShip);
        for (let i = 0; i < size; i++) {
          if (isHorizontalShip) {
            console.log('1st if', roundToNearest(xPosition) + gridCellSize);
            sections.push({
      ...
      setShipPositions((prevShipPositions) => {
        ...
        const updatedShipPositions = [
          ...prevShipPositions,
          {
            ...
            sections: generateSections(shipSize),
          },
        ];
